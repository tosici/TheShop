...
{
    public class Dealer1 : IDealerService
    {
        private readonly string _supplierUrl;

        public Dealer1(string url)
        {
            _supplierUrl = url; ;
        }
        ...
        public async Task<Article> BuyArticle(Article article)
        {
            using (var client = new HttpClient())
            {
                ...
                var response = client.PostAsync($"{_supplierUrl}/{article?.BuyerUserId}", content);
                var msg = await response;
                switch (msg.StatusCode)
                {
                    case HttpStatusCode.OK:
        ...
        public async Task<Article> GetArticle(int id)
        {
            using (var client = new HttpClient())
            {
                var response = client.SendAsync(new HttpRequestMessage(HttpMethod.Get, $"{_supplierUrl}/GetArticle/{id}"));
                var msg = await response;
                switch (msg.StatusCode)
                {
                    case HttpStatusCode.OK:
